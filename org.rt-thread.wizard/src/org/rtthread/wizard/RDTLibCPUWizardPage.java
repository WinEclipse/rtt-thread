 ...
 * @date 2011-9-2
 */
public class RDTLibCPUWizardPage extends WizardPage {

	private static final int SIZING_TEXT_FIELD_WIDTH = 250;

	private static final String SAVED_LOCATION_ATTR = "target";
	 ...
	 * @param rttPath
	 */
	protected RDTLibCPUWizardPage(String pageName, String rttPath) {
		super(pageName);
	...
	private void createGroup(Composite parent) {
		...
		tree.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				TreeItem libcpu = searchTreeItem(tree.getItems(), "libcpu");
				item = tis[0];
				text = item.getText();
				while (item != null && item.getParentItem() != libcpu) {
	...
	private void initData() {
		String path = "";
		if (null != rttPath && !"".equals(rttPath.trim())) {
			path = rttPath;
		} else {
			path = getDialogSettings().get(SAVED_RTTHREAD);
		}
		if (null != path && !"".equals(path)) {
			pathText.setText(path + "\\libcpu");
			updateTreeItems(path);
			TreeItem item = searchTreeItem(tree.getItems(), "libcpu");
			if (null != item)
				tree.select(item);
		}
	}
	...
	public URI getLocation() throws URISyntaxException {
		IPath path = new Path(getPathFromLocationField());
	...
	public String getTargetPath() {
		return targetText.getText().replace("\\", "/");
	}
}
